...
This engine consisting of discrete states, specifically engineered to manage the complexities of distributed transactions within the Saga pattern. This multi-state architecture serves as a persistent 'source of truth,' allowing the orchestrator to maintain strict contextual awareness at every execution checkpoint. By decoupling the transaction lifecycle into these granular phases, the engine ensures deterministic flow control and precise derivation of the next state—even across asynchronous boundaries.